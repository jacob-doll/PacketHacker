...
bool ByteEditor::EndEdit(int row, int col, const wxGrid *grid,
...
bool ByteEditor::IsAcceptedKey(wxKeyEvent &event)
{
    if (wxGridCellEditor::IsAcceptedKey(event))
    {
        ...
        if ((keycode < 128) &&
            (wxIsdigit(keycode) || keycode == 'a' || keycode == 'b' || keycode == 'c' || keycode == 'd' || keycode == 'e' || keycode == 'f'))
        {
            return true;
        }
    }
...
void ByteViewer::SetByte(int offset, uint8_t byte)
{
    int row = offset / 16;
